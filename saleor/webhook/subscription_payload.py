...
from graphql import parse, get_default_backend
...
def generate_payload_from_subscription(
        event_type: str,
        subscribable_object,
        subscription_query: str, context: HttpRequest, app: Optional[App]=None) -> Optional[Dict[str, Any]]:
    ...
    results = document.execute(
        ...
        root=(event_type, subscribable_object), context=context,
        middleware=graphql_middleware
    )
    if hasattr(results, "errors"):
        logger.warning(str(results.errors))
        logger.warning("Unable to build a payload for subscription", extra={"query": subscription_query, "app": app_id})
        return None
    payload = []
